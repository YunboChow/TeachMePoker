...
class HandTest {
     ...
     void toPowerBar() { //With 7 cards
          ...
          Assertions.assertTrue(hand.toPowerBar() >= 1 && hand.toPowerBar() <= 4);
     }
     ...
     void theHelp() { //With 5 cards
          ...
          Assertions.assertTrue(bestHands.contains(hand.theHelp()));
     }
     ...
     void theAdvice() { //With 5 cards
          ...
          Assertions.assertTrue(adviceList.contains(hand.theAdvice()) || hand.theAdvice().contains(advicesAddon[0]) ||
                    hand.theAdvice().contains(advicesAddon[1]));
     }

     @Test
     void Hand_WithInputAsNull() { //null
          Throwable exception = Assertions.assertThrows(Exception.class, () -> new Hand(null));
          Assertions.assertEquals("Cannot invoke \"java.util.ArrayList.size()\" because \"this.cards\" is null", exception.getMessage());
     }
     ...
     void Hand_WithInValidAmountOfCards() { //Less than 2 cards
          ...
          Throwable exception = Assertions.assertThrows(Exception.class, () -> new Hand(cards));
     ...
     void getHighlightedCards() { //7 cards
          ...
          Assertions.assertTrue(hand.getHighlightedCards() instanceof ArrayList); //Lite tamt men alternativet är att kolla om array size 0 till mängden kort som hand skapades med
     }
     ...
     void getHandStrength() { //6 cards
          ...
          Assertions.assertTrue(hand.getHandStrenght() >= 0 && hand.getHandStrenght() <= 8);
     }
